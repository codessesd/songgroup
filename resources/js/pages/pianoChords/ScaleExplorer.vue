    ...
    <div class="space-y-6 rounded-2xl border border-[#dcd8ce] bg-[#fffdf8] p-6 lg:col-span-2">
      <div class="grid gap-5 md:grid-cols-2">
        <div>
          ...
          <div class="grid grid-cols-6 gap-2">
            <button
              v-for="(_, index) in 12"
              :key="index"
              class="rounded-lg bg-[#f3f1eb] p-2 text-xs"
              :class="scaleRoot === index ? 'bg-[#e7b08c] text-[#173d3a]' : ''"
              @click="emit('scaleRootChange', index)"
            >
              {{ noteName(index) }}
            </button>
          </div>
        ...
        <div>
          ...
          <select
            :value="selectedScale"
            class="w-full rounded-xl border border-[#dcd8ce] bg-[#f3f1eb] p-3 text-sm"
            @change="emit('scaleChange', ($event.target as HTMLSelectElement).value)"
          >
      ...
      <div>
        ...
        <div class="flex flex-wrap gap-2">
          <div
            v-for="(offset, index) in currentScale.pattern"
            :key="offset"
            class="rounded-xl bg-[#f3f1eb] px-4 py-3 text-center"
          >
            <small class="block text-[#b56e4b]">Degree {{ index + 1 }}</small
            ><strong>{{ noteName(scaleRoot + offset) }}</strong>
          </div>
        </div>
      </div>
      <div>
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <button
            v-for="(offset, index) in currentScale.pattern"
            :key="offset"
            class="rounded-xl border border-[#dcd8ce] bg-[#f3f1eb] p-3 text-left"
            @click="emit('exploreChord', (scaleRoot + offset) % 12, chordTypeForDegree(index))"
          >
            <small class="block text-[#b56e4b]">Step {{ index + 1 }}</small
            ><strong>{{ noteName(scaleRoot + offset) }}</strong>
          </button>
        </div>
      </div>
